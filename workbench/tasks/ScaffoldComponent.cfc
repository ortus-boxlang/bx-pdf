...
component {
	...
	function init(){
		...
		variables.util
			.addMapping( '/componentsDirectory',	"#variables.root#/src/main/java/ortus/boxlang/modules/pdf/components" )
			.addMapping( '/componentsTestsDirectory', "#variables.root#/src/test/java/ortus/boxlang/modules/pdf/components" );
	...
	function run(
		...
		var bifTargetDirectory = expandPath( "/componentsDirectory" );
		if( !directoryExists( bifTargetDirectory ) ){
			directoryCreate( bifTargetDirectory );
		}
		if( arguments.withTests ){
			var testTargetDirectory = expandPath( "/componentsTestsDirectory" );
		...
		if( listLen( package, "." ) == 1 ){
			arguments.package = "ortus.boxlang.modules.pdf.components." & package;
		}
